...
//this method update four fields and check three field
public function update_fourfieldscheck3($tablename, $tableidfield, $tableid, $tableidfield1, $tableid1, $tableidfield2, $tableid2, $tablefield1,  $tablevalue1, $tablefield2,  $tablevalue2, $tablefield3,  $tablevalue3, $tablefield4,  $tablevalue4){

    $mysqli = $this->connect();
    
    $sql =  "UPDATE {$tablename} SET  {$tablefield1}=:tablevalue1, {$tablefield2}=:tablevalue2, {$tablefield3}=:tablevalue1, {$tablefield2}=:tablevalue2 WHERE {$tableidfield}=:tableid and {$tableidfield1}=:tableid1 and {$tableidfield2}=:tableid2";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':tablevalue2'=>$tablevalue2, ':tablevalue3'=>$tablevalue3, ':tablevalue4'=>$tablevalue4, ':tableid'=>$tableid, ':tableid1'=>$tableid1, ':tableid2'=>$tableid2])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
...
//this method update four fields and check three field
public function update_twofieldscheck2($tablename, $tableidfield, $tableid, $tableidfield1, $tableid1,  $tablefield1,  $tablevalue1, $tablefield2,  $tablevalue2){

    $mysqli = $this->connect();
    
    $sql =  "UPDATE {$tablename} SET  {$tablefield1}=:tablevalue1, {$tablefield2}=:tablevalue2 WHERE {$tableidfield}=:tableid and {$tableidfield1}=:tableid1";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':tablevalue2'=>$tablevalue2, ':tableid'=>$tableid, ':tableid1'=>$tableid1])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
...
//this method update one fields and check two field
public function update_onefieldscheck2($tablename, $tableidfield, $tableid, $tableidfield1, $tableid1,  $tablefield1,  $tablevalue1){

    $mysqli = $this->connect();
    
    $sql =  "UPDATE {$tablename} SET  {$tablefield1}=:tablevalue1 WHERE {$tableidfield}=:tableid and {$tableidfield1}=:tableid1 ";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':tableid'=>$tableid, ':tableid1'=>$tableid1])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
...
public function update_threefields($tablename, $tableidfield, $tableid, $tablefield1,  $tablevalue1, $operatoridfield, $operatorid, $updatefield, $udate){

    $mysqli = $this->connect();
    
    $sql =  "UPDATE {$tablename} SET  {$tablefield1}=:tablevalue1, {$operatoridfield}=:operatorid, {$updatefield}=:udate WHERE {$tableidfield}=:tableid";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':operatorid'=>$operatorid, ':udate'=>$udate, ':operatorid'=>$operatorid, ':tableid'=>$tableid])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
