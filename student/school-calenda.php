	...
	$(document).ready(function() {
		...
		$('#calendar').fullCalendar({
			...
			events: [
				<?php
					$k=0;
					 $studentsattend=$SHstudent->allstudent('schoolcalenda', 'startdate', 'ASC');
					 $attendcount=count($studentsattend);
                        if(is_array($studentsattend)) {
                         foreach($studentsattend as $studentsattendrec1){
							$k = $k + 1;
							$attendancedate = $studentsattendrec1["startdate"];
						...
						?>
							...
							{      
								...
								title:  '<?php echo $studentsattendrec1["title"]. " ". $studentsattendrec1["kickoftime"]." ".$studentsattendrec1["description"]; ?>',
