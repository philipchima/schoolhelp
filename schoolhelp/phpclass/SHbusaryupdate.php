...
class updateTable extends Dbh{
	...
	public function delete_bursary($tablename, $fieldname, $fieldvalue){
    $mysqli = $this->connect();
...
public function update_twofields($tablename, $tableidfield, $tableid, $tablefield1,  $tablevalue1,  $tablefield2,  $tablevalue2){

    $mysqli = $this->connect();
    
    $sql =  "UPDATE {$tablename} SET  {$tablefield1}=:tablevalue1, {$tablefield2}=:tablevalue2 WHERE {$tableidfield}=:tableid";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':tablevalue2'=>$tablevalue2,  ':tableid'=>$tableid])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
...
//Update nine fields
public function update_eightfields($tablename, $tableidfield, $tableid, $tablefield1,  $tablevalue1, $tablefield2, $tablevalue2, $tablefield3, $tablevalue3, $tablefield4, $tablevalue4, $tablefield5, $tablevalue5, $tablefield6, $tablevalue6, $tablefield7, $tablevalue7, $operatoridfield, $operatorid){

    $mysqli = $this->connect();
    
    $sql =  "UPDATE {$tablename} SET  {$tablefield1}=:tablevalue1, {$tablefield2}=:tablevalue2, {$tablefield3}=:tablevalue3, {$tablefield4}=:tablevalue4, {$tablefield5}=:tablevalue5, {$tablefield6}=:tablevalue6, {$tablefield7}=:tablevalue7, {$operatoridfield}=:operatorid WHERE {$tableidfield}=:tableid";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':tablevalue2'=>$tablevalue2, ':tablevalue3'=>$tablevalue3, ':tablevalue4'=>$tablevalue4, ':tablevalue5'=>$tablevalue5, ':tablevalue6'=>$tablevalue6, ':tablevalue7'=>$tablevalue7, ':operatorid'=>$operatorid, ':operatorid'=>$operatorid, ':tableid'=>$tableid])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
