...
//Getting details from two different tables
 if($section=="state") {
  ...
  $retrievedata=$schoolhelpDash->alldashedit($tablename, $fieldname, $fieldvalue);
...
//Getting details from two different tables
 if($section=="retrieveselection") {
 	...
 	//($tablename, $fieldname, $fieldvalue, $fieldrequested)
 	$retrievedata=$schoolhelpDash->allwebedit($tablename, $fieldname, $fieldvalue);
...
//Getting details from two different tables
 if($section=="twofieldscheck") {
  ...
  //($tablename, $fieldname, $fieldvalue, $fieldrequested)
  $retrievedata=$schoolhelpDash->alldashedit2($tablename, $fieldname, $fieldvalue, $fieldname1, $fieldvalue1);
  if (is_array($retrievedata)) {
    echo 1;
  ?>                
 ...
 $fieldname=trim(isset($_POST['fieldname'])?$_POST['fieldname']:false);
  ...
  <?php
    foreach($retrievedata as $field){
      ...
      $departmentmethod=$schoolhelpDash->alldashedit('department', 'did', $departmentid);
