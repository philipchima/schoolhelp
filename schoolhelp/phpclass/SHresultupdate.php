...
public function deletecheck5($tablename, $fieldname, $fieldvalue, $fieldname1, $fieldvalue1, $fieldname2, $fieldvalue2, $fieldname3, $fieldvalue3, $fieldname4, $fieldvalue4){
    $mysqli = $this->connect();

     $sql = "DELETE FROM {$tablename} WHERE {$fieldname}='$fieldvalue' and {$fieldname1}='$fieldvalue1'  and {$fieldname2}='$fieldvalue2'  and {$fieldname3}='$fieldvalue3' and {$fieldname4}='$fieldvalue4' ";
...
//Update ten fields
public function update_eightfields($tablename, $tableidfield, $tableid, $tablefield1,  $tablevalue1, $tablefield2, $tablevalue2, $tablefield3, $tablevalue3, $tablefield4, $tablevalue4, $tablefield5, $tablevalue5, $tablefield6, $tablevalue6, $tablefield7, $tablevalue7, $tablefield8, $tablevalue8){

    $mysqli = $this->connect();
    
    $sql =  "UPDATE {$tablename} SET  {$tablefield1}=:tablevalue1, {$tablefield2}=:tablevalue2, {$tablefield3}=:tablevalue3, {$tablefield4}=:tablevalue4, {$tablefield5}=:tablevalue5, {$tablefield6}=:tablevalue6, {$tablefield7}=:tablevalue7, {$tablefield8}=:tablevalue8 WHERE {$tableidfield}=:tableid";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':tablevalue2'=>$tablevalue2, ':tablevalue3'=>$tablevalue3, ':tablevalue4'=>$tablevalue4, ':tablevalue5'=>$tablevalue5, ':tablevalue6'=>$tablevalue6, ':tablevalue7'=>$tablevalue7, ':tablevalue8'=>$tablevalue8, ':tableid'=>$tableid])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
...
//Updating Status in a table
 ...
 public function update_scorestatus1($scoretblname, $statusfield, $status, $operatoridfield, $operatorid, $updatefield, $udate, $sessionid, $semesterid, $levelid, $optionid){

    $mysqli = $this->connect();
    
    $sql ="UPDATE {$scoretblname} SET  {$statusfield}=:status, {$operatoridfield}=:operatorid, {$updatefield}=:udate WHERE sessionid=:sessionid and semesterid=:semesterid and levelid=:levelid and optionid=:optionid";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':status'=>$status, ':operatorid'=>$operatorid, ':udate'=>$udate,  ':sessionid'=>$sessionid, ':semesterid'=>$semesterid, ':levelid'=>$levelid, ':optionid'=>$optionid])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
...
public function update_fourfields($tablename, $tableidfield, $tableid, $tablefield1,  $tablevalue1,  $tablefield2,  $tablevalue2, $operatoridfield, $operatorid, $updatefield, $udate){
    ...
    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':tablevalue2'=>$tablevalue2, ':operatorid'=>$operatorid, ':udate'=>$udate,  ':tableid'=>$tableid])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
...
//this method is used to update early class comment
public function update_fivefields($tablename, $tableidfield, $tableid, $tablefield1,  $tablevalue1,  $tablefield2,  $tablevalue2, $tablefield3,  $tablevalue3, $operatoridfield, $operatorid, $updatefield, $udate){

    $mysqli = $this->connect();
    
    $sql =  "UPDATE {$tablename} SET  {$tablefield1}=:tablevalue1, {$tablefield2}=:tablevalue2, {$tablefield3}=:tablevalue3, {$operatoridfield}=:operatorid, {$updatefield}=:udate WHERE {$tableidfield}=:tableid";
    
    // Prepare statement
     $stmt = $mysqli->prepare($sql);

    if($stmt->execute([':tablevalue1'=>$tablevalue1, ':tablevalue2'=>$tablevalue2, ':tablevalue3'=>$tablevalue3, ':operatorid'=>$operatorid, ':udate'=>$udate, ':operatorid'=>$operatorid, ':tableid'=>$tableid])){
        return "Success";
    } else {
        return "failed: " . $mysqli->error;
    }
...
public function delete_result($tablename, $fieldname, $fieldvalue){
    ...
    //Deleting a 4 record
public function delete_result4($tablename, $fieldname, $fieldvalue, $fieldname1, $fieldvalue1, $fieldname2, $fieldvalue2, $fieldname3, $fieldvalue3){
    $mysqli = $this->connect();

     $sql = "DELETE FROM {$tablename} WHERE {$fieldname}='$fieldvalue' and {$fieldname1}='$fieldvalue1' and {$fieldname2}='$fieldvalue2' and {$fieldname3}='$fieldvalue3'";
