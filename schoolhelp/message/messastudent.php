...
if ($page==2) {
  ...
  $messagetext= trim(isset($_POST["stumessage"])?$_POST["stumessage"]:false);
  $categoryid=0;
  $odate=date("Y-m-d");
  ...
  if ($levelid==10000) {
    $departmentalid=10000;
  }else{
   // Getting the Department ID
    $deptdata=$schoolhelpDH->allmessageedit('level', 'levelid', $levelid);
    if (is_array($deptdata)) {
      foreach($deptdata as $deptrec){
        $departmentalid=trim($deptrec['departmentid']);
  ...
  //Making sure that number is not empty
  if ($stuid!="") {
   ...
   $state=$tableInsert->insert_9fields('sentbulkmsg', 'departmentid', $departmentalid, 'levelid', $levelid, 'categoryid', $categoryid, 'messagetitle', $messagetitle, 'messagecontent', $messagetext,  'phonenoinvolved', $bulkno, 'phonenocount', $nocount, 'operatorid', $schoolhelp,  'odate', $odate);

    $smsclass->sendSMS($bulkno, $messagetext,"Swiftotech", 0);

    $sql="Messages has be sent the following nummber: ".$bulkno;
    $page="";
    
   
    echo "<script>
        window.location.href='?schoolhelp=$schoolhelp&sql=$sql&page=$page';
      </script>";
                    ...
                    <table id="datatable-buttons" style="width:100%" class="table table-striped table-bordered table-responsive">
                      ...
                      <tbody>
                        <?php $k=0; 
                              ...
                              $records=$schoolhelpDH->allmessageeditorder('sentbulkmsg', 'categoryid', '0', 'sentbulkmsgid', 'DESC');
                              if (is_array($records)) {
                              ...
                              foreach($records as $fieldrecord){
                                ...
                                $levelid=trim($fieldrecord['levelid']);
                                 $phonenocount=trim($fieldrecord['phonenocount']);
                                $categoryname="Student";


                                
                                $messagetitle=trim($fieldrecord['messagetitle']);
                                $messagecontent=trim($fieldrecord['messagecontent']);
                              ...
                              if ($admintype==0) {
                                ...
                                if ($departmentid==$logindepartmentid) {?>
                                      ...
                                      <tr data-toggle="tooltip" data-placement="top"  required="required" data-html="true" title="<em> <ul >
                                        ...
                                        <td><?php echo  substr($messagetitle,0, 12); ?></td>
                                        <td><?php echo  substr($messagecontent,0, 12); ?></td>
                                         <td><?php echo  $phonenocount; ?></td>
