  ...
  foreach($stuid as $i => $sdentid){
    ...
    $status=0;
     if ($newlevelid==1000) {
      ...
      if ($presencestatus>1) {
        //mysqli_query($mysqli, "UPDATE promotion set oldclassid='$oldclass', newclassid='$newclass', sid='$sdentid', passmark='$passmark', average='$studentave', year='$year', cgroup='$cgroup', oldgroup='$oldgroup', user='$xID', xdate='$date' WHERE pid='$presencestatus' ") or die("could not insert, reason: ".mysqli_error($mysqli));
        ...
        //mysqli_query($mysqli, "INSERT into promotion set oldclassid='$oldclass', newclassid='$newclass', sid='$sdentid', passmark='$passmark', average='$studentave', year='$year', cgroup='$cgroup', oldgroup='$oldgroup', user='$xID', xdate='$date' ") or die("could not insert, reason: ".mysqli_error($mysqli));
        ...
        //mysqli_query($mysqli, "UPDATE students set class='$newclass', group_id='$cgroup', status='$status' where stid='$sdentid'") or die("could not insert, reason: ".mysqli_error($mysqli));
                      ...
                      <tbody>
                        <?php $k=0;
                           ...
                           <?php  
                            ...
                            $records=$SHResultOOP->allresultedit3('failedstudents', 'oldlevelid', $levelid, 'oldoptionid', $optionid, 'sessionid', $sessionid);
                              if (is_array($records)) {
                              ...
                              $average=trim($fieldrecord['average']);
                              $passmark=trim($fieldrecord['passmark']);
               ...
               <td>class: </td>
                <td >
                  <select name="newlevelid" id="newlevelid"  class="form-control col-md-6 col-xs-12"  onchange="return loadoptionpromote('optiontable', 'levelid', this.value, 'loadoptionpromote', 'grouppromote')" required="required">
                        ...
                        <?php
                          $promolevel=$SHResultOOP->allresultedit('level', 'levelid', $newlevelid);
                          if (is_array($promolevel)) {
                          
                              $k = 0;
                          ?>
                        <?php  foreach($promolevel as $promolevelrec){   ?>
                        <option value="<?php echo  trim($promolevelrec['levelid']); ?>" <?php if (trim($promolevelrec['levelid'])==$newlevelid){ ?> selected="selected" <?php } ?>><?php echo  $promolevelrec['levelname']?></option>
                        <?php  } 
                      } ?>
                      </select>
                   ...
                   <td style="padding-left:10px" >Option|Arm: </td>
                     ...
                     <?php  $retrievedata=$SHResultOOP->allresultedit('optiontable', 'optid', $newoptionid); ?>  
                     <select  name="newoptionid" id="newoptionid" required="required"  class="form-control col-md-6 col-xs-12" onchange="enablingbutton('promotbutton');">
                       <option value="">--Select Option|Arm--</option>
                        ...
                        ?>
                              <option value="<?php echo $field['optid']; ?>" <?php if (trim($field['optid'])==$newoptionid){ ?> selected="selected" <?php } ?> ><?php echo $field['optname']; ?></option>
                        <?php
                            }
                          }
                  ...
                  <td> <input  type="submit" class="btn btn-primary" value="Promote Student" id="promotbutton"  /></td>
             </tr>
