...
if ($pageaccess!=1) {
  ...
  exit();
	...
	// Show student position in the result
    $resultpositiongrade="";
    $activationrecords2=$SHResultOOP->allresultedit('resultactivations', 'titlename', 'Singular Result');
...
$positiontblname=trim("positionresult".$departmentid);
	...
	$htitlelabel="#063";
	$htitle="black";
	$tableheadbgcolor="green";
	$tablecontentcolor1	="";
	$tablecontentcolor2	="t#d2dc2a";
	$resultbackground="";
}
...
$html .=$div.$divbackground.'
        	...
        	<td colspan="3" width="100%">
				<table width="100%">
					<tr>
						...
						<td width="20%" align="right"><img src="'.$pic.'" width="100px" height="110px"></td>
					</tr>
				</table>
			</td>
		</tr>
                	...
                	<tr>
                    	<td width="100%">
                        	<table width="100%">
                                ...
                                <tr><td align="left" style="color:'.$htitlelabel.'">Date of Birth:</td><td align="left" style="border-bottom:2px solid black; color:'.$htitle.'"><b> '. date("l jS F, Y", strtotime($dateofbirth)).'</b></td></tr>
    ...
    //$select_contentass=("SELECT * from assessment where departmentid ='$departmentid' ");
	//$content_resultass= mysqli_query($mysqli, $select_contentass) or die (mysqli_error($mysqli));
	//$contentass = mysqli_fetch_assoc($content_resultass);
    ...
    //do {
    	//$asscount[]=$contentass["sid"];
    //$html.='<th  style="text-align:center; width:auto"><span class="vertical" >'.$contentass['type'].'('.$contentass['per'].'%)'.'</span></th>';
		//}while($contentass = mysqli_fetch_assoc($content_resultass));
   ...
   if ($semesterid==3) { 
    ...
    <th  style="text-align:center; width:auto">Remark</th>
 		 ...
 		 //$select_content=("select * from subjects where class ='$departmentid'");
				...
				if (!is_array($content1)) {
                     ...
                     echo  "No Subject Found";
                        ...
                        $subjecttodisplay=$SHResultOOP->subtable($sessionid, $semesterid, $stuid, $levelid, $subjectid, $scoretablename);
                        ...
                        $html.='</tr>';
                    	 ...
                    	 //} while($content = mysqli_fetch_assoc($content_result));
                    	 	...
                    	 	$html.='<tr>'; if ($semesterid==2) { $html.='<td>&nbsp</td>'; } if ($semesterid==3) { $html.='<td>&nbsp</td><td>&nbsp</td>'; } $html.='<td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td></tr>';
                    	 	$html.='<tr>'; if ($semesterid==2) { $html.='<td>&nbsp</td>'; } if ($semesterid==3) { $html.='<td>&nbsp</td><td>&nbsp</td>'; } $html.='<td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td></tr>';
                    	 	$html.='<tr>'; if ($semesterid==2) { $html.='<td>&nbsp</td>'; } if ($semesterid==3) { $html.='<td>&nbsp</td><td>&nbsp</td>'; } $html.='<td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td></tr>';
                    	 	...
                    	 	$html.='<td>'.$supaccasssubjtotalscore.'</td><td>'.$stualltermoverallcumu.'</td><td></td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td></tr>';
                    	 	$html.='<tr><td style="font-size:11px; font-weight:bold; font-family: Arial ; ">CUMULATIVE(%)</td>'; if ($semesterid==2) { $html.='<td>&nbsp</td>'; } if ($semesterid==3) { $html.='<td>&nbsp</td><td>&nbsp</td>'; } $html.='<td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>&nbsp</td><td>'.$studavescore.'%</td><td>'.$acchighestsubjtermavescore.'%</td><td>'.$accscore4allstuallsub2.'%</td><td>&nbsp</td></tr>';
		...
		$html.='<td style="width:50%" valign="top"><table valign="top" style="width:100%; height:35%; margin:2px; padding:2px; border-collapse:collapse;" cellspacing="0px" cellpadding="0px">
		<tr>
		...
		$html.='<tr><td style="width:50%; ">'.$domainrec["domainname"].'</td><td style="width:10%;"><center>'; if($domainstate==5) { $html.= '<img style="width:30%" src="../images/check-mark.png" />'; } $html.='</center></td><td style="width:10%"><center>'; if($domainstate==4) { $html.= '<img style="width:30%" src="../images/check-mark.png" />'; } $html.='</center></td><td style="width:10%"><center>'; if($domainstate==3) { $html.= '<img style="width:30%" src="../images/check-mark.png" />'; } $html.='</center></td><td style="width:10%"><center>'; if($domainstate==2) { $html.= '<img style="width:30%" src="../images/check-mark.png" />'; } $html.='</center></td><td style="width:10%"><center>'; if($domainstate==1) { $html.= '<img style="width:30%" src="../images/check-mark.png" />'; } $html.='</center></td></tr>';
		}
	}
		$html.='</table>
		</td>
		...
		//Signatory Person
	if ($principlec=="") {
			$principlec=$SHResultOOP->listselection('commentlist', $departmentid, $totalscore);
		}
		...
		<table style="width:100%"><tr>';   $pic = "../uploads/signatures/". $teachersignature;
							   $pic2= trim($teachersurname. " " .$teacherothername);
					   $html .='<td style="width:35%" align="center" >';
					   		if (file_exists("../uploads/signatures/".$teachersignature)) {
						...
						$html .='<td align="center" width="50%">';
							   if(file_exists("../uploads/signatures/".$positionsignature)){ 
							   		$sign="../uploads/signatures/".$positionsignature;
