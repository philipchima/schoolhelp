...
$setupinventoryid=isset($_POST['setupinventoryid'])?$_POST['setupinventoryid']:false;


$state="Completely Unsuccessfully";

foreach($setupinventoryid as $typecount=>$typenames){

$name=trim($typenames);
$description=trim(isset($_POST['description'.$typecount])?$_POST['description'.$typecount]:false);
$inventorynameid=trim(isset($_POST['inventorynameid'.$typecount])?$_POST['inventorynameid'.$typecount]:false);
$description=trim(isset($_POST['description'.$typecount])?$_POST['description'.$typecount]:false);
$noofitems=trim(isset($_POST['noofitems'.$typecount])?$_POST['noofitems'.$typecount]:false);
$noofitempackages=trim(isset($_POST['noofitempackages'.$typecount])?$_POST['noofitempackages'.$typecount]:false);
$amountperitem=trim(isset($_POST['amountperitem'.$typecount])?$_POST['amountperitem'.$typecount]:false);

//Checking Whetter this inventory has been added
 $records=$SHbusaryOOP->allbusaryedit2('setupinventoryid', $name, 'inventorynameid', $inventorynameid);
    if (is_array($records)) {
...
$insertedrec+=1;
$state=$tableInsert->insert_6fields('inventoryname', 'setupinventoryid', $setupinventoryid, 'inventoryname', $name, 'description', $description, 'operatorid', $schoolhelp, 'udate', $udate, 'odate', $odate);
...
if($page==4) {
    $sql="Criminal Suspected";
  if ($dashedit_d==1) {
  $inventorynameid=trim(isset($_POST['inventorynameid'])?$_POST['inventorynameid']:false);
...
$state=$tableUpdate->update_threefields('inventoryname', 'inventorynameid', $inventorynameid, 'setupinventoryid', $setupinventoryid, 'inventoryname',  $inventoryname, 'description', $description, 'operatorid', $schoolhelp);
...
if ($page==6) {
  ...
  if ($dashdelete_d==1) {
   ...
   $inventorynameid=trim(isset($_GET['id'])?$_GET['id']:false);
   //$photo=trim(isset($_GET['photo'])?$_GET['photo']:false);
   $tableUpdate= new updateTable;
    $state=$tableUpdate->delete_timetable('inventoryname', 'inventorynameid', $inventorynameid);
              ...
              <div class="col-md-12 col-sm-12 col-lg-12 col-xs-12" style="margin-bottom: 20px;">
                <div class="x_panel">
                    ...
                    <table id="datatable-buttons" style="width:100%" class="table table-striped table-bordered table-responsive">
                      ...
                      <tbody>
                        <?php $k=0; 
                              ...
                              $records=$SHbusaryOOP->allbusary('inventoryitems', 'inventoryitemsid', 'DESC');
                              if (is_array($records)) {
                                $k1=0;
                              foreach($records as $fieldrecord){
                                ...
                                $inventorynameid=trim($fieldrecord['inventorynameid']);

                              $records1=$SHbusaryOOP->allbusaryedit('inventoryname', 'inventorynameid', $inventorynameid);
                              if (is_array($records1)) {
                                
                                foreach($records1 as $fieldrecord1){
                                  $inventoryname=trim($fieldrecord1["inventoryname"]);
                                  $setupinventoryid=trim($fieldrecord1["setupinventoryid"]);
                                }
                              ...
                              //Getting Admin Detials
                                ...
                                //collecting setupinventory  record
                                  $inventorycategory="";
                                  $setupinventoryrec=$SHbusaryOOP->allbusaryedit('setupinventory', 'setupinventoryid', trim($setupinventoryid));
                                  if (is_array($setupinventoryrec)) {
                                     foreach($setupinventoryrec as $setupinventoryrecord){
                                        $inventorycategory=$setupinventoryrecord['inventoryname'];
                                        $departmentid=trim($setupinventoryrecord['departmentid']);
                                     }
                                   }
                                ...
                                ?>
                                      <tr>
                                        ...
                                        <td><?php echo $inventorycategory; ?></td> 
                                                                    
                                        <td><?php echo  $fieldrecord['inventoryname']; ?></td>
                                        <td><?php echo  substr($fieldrecord['description'], 0, 12); ?></td>
                                        <td><?php echo  substr($fieldrecord['totalamount'], 0, 12); ?></td>
                                        <td><?php echo  substr($fieldrecord['paidamount'], 0, 12); ?></td>
                                        <td><?php echo  substr($fieldrecord['balamount'], 0, 12); ?></td>
                                     
                                         <td><button onclick="funcprint('<?php echo $schoolhelp; ?>','<?php echo $fieldrecord['inventorynameid']; ?>')"><center><i class="fa fa-print" style="color:green; font-size:2em"></i></center></button></td>
                                          <?php if ($dashedit_d==1) { ?>
                                        <td><button onclick="funcedit('<?php echo $schoolhelp; ?>','<?php echo $fieldrecord['inventorynameid']; ?>')"><center><i class="fa fa-edit" style="color:#d2dc2a; font-size:2em"></i></center></button></td>
                                        <?php } ?>
                                         <?php if ($dashdelete_d==1) { ?>
                                        <td><button onclick="funcdelete('<?php echo $schoolhelp; ?>','<?php echo $fieldrecord['inventorynameid']; ?>','');"><center><i class="fa fa-trash" style="color:red; font-size:2em"></i></center></button></td>
                                        <?php } ?>
                  ...
                  <form enctype="multipart/form-data" action="?page=2&schoolhelp=<?php echo $schoolhelp; ?>" method="POST"  id="frmmedtreatmentid"  class="form-horizontal form-label-left" >
                     ...
                     <?php $fieldcount=0; $u=1; while($numberoffields>=1){
                       $numberoffields-=1;  ?>
                       <?php echo $pagename; ?> <?php echo $u; ?>
                       <hr>

                       <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="departmentid">Select Item Category<span class="required">*</span>
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                         
                          <select  id="setupinventoryid[]" required="required" name="setupinventoryid[]" class="form-control col-md-7 col-xs-12" onchange="retrieveselection('inventoryname', 'setupinventoryid', this.value, '<?php echo $fieldcount; ?>', 'retrieveselection', 'retrieveselection',  'inventorynameid', 'inventoryname', 'inventoryname', 'Inventory Name', 'Please check whether Inventory Name has been added to this Inventory Category');">
                            <option value="">--Select Item Category--</option>
                            
                            <?php
                            $deptrecord=$SHbusaryOOP->allbusary('setupinventory', 'setupinventoryid', 'ASC');
                            if (is_array($deptrecord)) {
                           
                            foreach($deptrecord as $deptdata){
                              $departmentid=trim($deptdata['departmentid']);
                              if ($admintype==0) {
                                  if ($departmentid==$logindepartmentid or $departmentid==0) {
                            ?>
                            <option value="<?php echo $deptdata['setupinventoryid']; ?>" ><?php echo $deptdata['inventoryname']; ?></option>
                            <?php 
                                }
                                  }//end of checking whether someone is a super admin
                                  else{ ?>
                              <option value="<?php echo $deptdata['setupinventoryid']; ?>" ><?php echo $deptdata['inventoryname']; ?></option>
                         <?php
                            }
                     ...
                     <div id="retrieveselection">
                        <div class="form-group">
                             <label class="control-label col-md-3 col-sm-3 col-xs-12" for="inventorynameid">Inventory Name<span class="required">*</span>
                             </label>
                             <div class="col-md-6 col-sm-6 col-xs-12">
                             
                           <select  name="inventorynameid<?php echo $count; ?>" id="inventorynameid<?php echo $count; ?>" required="required"  class="form-control col-md-6 col-xs-12" >
                             <option value="">--Please select inventory category to enable you select inventory name--</option>
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <textarea id="description" class="form-control col-md-7 col-xs-12"  name="description<?php echo $fieldcount; ?>" placeholder="Please describe the Inventory"></textarea>
                        </div>
                      </div>
                       ...
                       <div class="form-group">
                        <label for="noofitempackages" class="control-label col-md-3 col-sm-3 col-xs-12">No of Item in a Package<span class="required"></span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" class="form-control" name="noofitempackages<?php echo $fieldcount; ?>" placeholder="Please enter number of item in the Package" id="noofitempackages<?php echo $fieldcount; ?>">
                        </div>
                      </div>

                       <div class="form-group">
                        <label for="noofitems" class="control-label col-md-3 col-sm-3 col-xs-12">No of Items<span class="required"></span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" class="form-control" name="noofitems<?php echo $fieldcount; ?>" placeholder="Please enter number of item you want to add" id="noofitems<?php echo $fieldcount; ?>">
                        </div>
                      </div>


                      <div class="form-group">
                        <label for="amountperitem" class="control-label col-md-3 col-sm-3 col-xs-12">Amount Per Item<span class="required"></span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" class="form-control" name="amountperitem<?php echo $fieldcount; ?>" placeholder="Please enter amount of an item" id="amountperitem<?php echo $fieldcount; ?>">
                        </div>
                      </div>

                      <div class="form-group">
                        <label for="totalamount" class="control-label col-md-3 col-sm-3 col-xs-12">Total Amount <span class="required"></span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" class="form-control" name="totalamount<?php echo $fieldcount; ?>" placeholder="Please enter total amount of this inventory" id="totalamount<?php echo $fieldcount; ?>">
                        </div>
                      </div>

                      <div class="form-group">
                        <label for="paidamount" class="control-label col-md-3 col-sm-3 col-xs-12">Paid Amount<span class="required"></span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" class="form-control" name="paidamount<?php echo $fieldcount; ?>" placeholder="Please enter the paid amount" id="amountpaid<?php echo $fieldcount; ?>">
                        </div>
                      </div>

                      <div class="form-group">
                        <label for="balanceamount" class="control-label col-md-3 col-sm-3 col-xs-12">Balance<span class="required"></span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input type="number" class="form-control" style="border:none" name="balanceamount<?php echo $fieldcount; ?>" >
                        </div>
                      </div>
                 ...
                 <?php } ?>
                  ...
                  if($page==3) {
                  $inventorynameid=trim(isset($_GET['id'])?$_GET['id']:false);
                  $records=$SHbusaryOOP->allbusaryedit('inventoryname', 'inventorynameid', $inventorynameid);
                            if (is_array($records)) {
                                
                              foreach($records as $fieldrecord){
                                $description=trim($fieldrecord['description']);
                                $inventoryname=trim($fieldrecord['inventoryname']);
                                $setupinventoryid=trim($fieldrecord['setupinventoryid']);
                  ...
                  <form enctype="multipart/form-data" action="?page=4&schoolhelp=<?php echo $schoolhelp; ?>" method="POST"  id="forminstitutionid"  class="form-horizontal form-label-left" >
                    <input id="inventorynameid" class="form-control col-md-7 col-xs-12" type="hidden" name="inventorynameid" required="required" value="<?php echo $inventorynameid; ?>">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          ...
                          <select  id="setupinventoryid" required="required" name="setupinventoryid" class="form-control col-md-7 col-xs-12" >
                            <option value="">--Select Item Category--</option>
                            ...
                            foreach($deptrecord as $deptdata){
                              ...
                              if ($admintype==0) {
                                  if ($departmentid==$logindepartmentid or $departmentid==0) {
                            ?>
                            <option value="<?php echo $deptdata['setupinventoryid']; ?>" <?php if (trim($deptdata['setupinventoryid'])==$setupinventoryid) {?> selected="selected" <?php } ?>><?php echo $deptdata['inventoryname']; ?></option>
                            <?php 
                                }
                     ...
                     <div class="form-group">
                        <label for="inventoryname" class="control-label col-md-3 col-sm-3 col-xs-12">Name<span class="required">*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <input id="inventoryname" class="form-control col-md-7 col-xs-12" placeholder="please enter Inventory Name" name="inventoryname" required="required" type="text" value="<?php echo $inventoryname; ?>">
                        </div>
                      </div>
                ...
                <?php if ($page==5) {
                   ...
                   $records=$SHbusaryOOP->allbusaryedit('inventoryname', 'inventorynameid', $inventorynameid);
                              ...
                              foreach($records as $fieldrecord){
                                ...
                                $setupinventoryid=trim($fieldrecord['setupinventoryid']);
                                  ...
                                  //collecting setupinventory  record
                                  $inventorycategory="";
                                  $setupinventoryrec=$SHbusaryOOP->allbusaryedit('setupinventory', 'setupinventoryid', trim($fieldrecord['setupinventoryid']));
                                  if (is_array($setupinventoryrec)) {
                                     foreach($setupinventoryrec as $setupinventoryrecord){
                                        $inventorycategory=$setupinventoryrecord['inventoryname'];
                                        $departmentid=trim($setupinventoryrecord['departmentid']);
                  ...
                  ?>
                    <div class="x_panel" >
                  <div class="x_title">
                    <h3><?php echo $inventoryname; ?> Time Type Details </h3>
                          ...
                          <div class="table-responsive">
                            <table class="table">
                              <tbody>
                                  ...
                                  <tr>
                                    <th style="width:50%">Inventory Category</th>
                                    <td><?php echo $inventorycategory; ?></td>
                                  </tr>
                                  <tr>
                                    <th style="width:50%">Inventory Name</th>
