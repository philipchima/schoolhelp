...
<?php
	if ($page == 2)
		{
			...
			$coursematerials=$_FILES["coursematerials"]["name"];
           if($coursematerials!=""){
              $target_dir = "../schoolhelp/uploads/coursematerials/";
              $papertmp=$_FILES['coursematerials']['tmp_name']; 
            $temp = explode(".", $_FILES["coursematerials"]["name"]);
           
            $exampapername =$staffid.round(microtime(true)) . '.' . strtolower(end($temp));
           move_uploaded_file($_FILES["coursematerials"]["tmp_name"], $target_dir . $exampapername);
            }

              
              $state=$teachersinsert->insert_10fields('coursematerials', 'levelid', $levelid, 'optionid', $optionid, 'semesterid', $semesterid, 'sessionid', $sessionid, 'instructorudate', $instructorudate, 'instructorid', $staffid, 'courseid', $courseid, 'dlink',  $exampapername, 'description',  $description, 'odate', $odate);
	...
	echo "
			<script language='javascript'>
				location.href='?refno=$staffid&sql=$sql&tcid=$classinfo'
			</script>
		";			
...
<?php
	if ($page == 4)
	
		{
			...
			$instructorudate=date("Y-m-d H:i:s");
            $cm_id=trim(isset($_POST['cm_id'])?$_POST['cm_id']:false);
            $description=trim(isset($_POST['description'])?$_POST['description']:false);
...
//Checking whether logo was uploaded(browsed)
  if($coursematerials!=""){
  $target_dir = "../schoolhelp/uploads/coursematerials/";
  $exampapertmp=$_FILES['coursematerials']['tmp_name']; 
  $temp = explode(".", $_FILES["coursematerials"]["name"]);
  $exampapername =strtolower($coursematerials).round(microtime(true)) . '.' . end($temp);
  move_uploaded_file($_FILES["coursematerials"]["tmp_name"], $target_dir. $exampapername);
...
$state= $schoolhelpupdate->update_threefields('coursematerials', 'cm_id', $cm_id, 'description',  $description,  'dlink',  $exampapername,  'instructorudate', $instructorudate);
			$sql=$state.":: Update Made, affected records = 1";
...
<?php
   ...
   $filename=trim(isset($_GET['filename'])?$_GET['filename']:false);
    ...
    $state=$schoolhelpupdate->delete_result('coursematerials', 'cm_id', $recordid);
...
?>
	...
	if ($page == "")
	{
		?>
        <div class="row">
				 <div class=" table-responsive" style="background:#063; margin:0% 2% 0% 4%; border-bottom:2px solid #063; color:#FFF; border-radius:2px; -moz-border-radius:3px; -o-border-radius:3px; -webkit-border-radius:3px; -o-border-radius:3px; width:88%; font-size:1.3em; margin-left:auto; margin-right:auto; text-align:center;">Course Material Document Uploads for  <?php echo $levelname . " " . $optionname." on ". $csname;?> 
        </div>
        </div>
      	<div class="row">
        <div style=" width:80%; margin-left:auto; margin-right:auto; margin-top:2%;"><?php if($sql){ echo "<span class='glyphicon glyphicon-info-sign' style='color:green'></span><span style='color:green'>$sql</span>";  }?></div>
			...
			<?php
               $teacherquestion=$SHteacher->teacheredit5order('coursematerials', 'courseid', $courseid, 'levelid', $levelid, 'optionid', $optionid, 'semesterid', $semesterid, 'sessionid', $sessionid, 'odate', 'DESC');
            ...
            ?> 
                ...
                <span id="colquesid" hidden="true"><?php echo $teacherquestionrec['cm_id']; ?></span>
                ...
                <tr >  
                 ...
                 if($teacherquestionrec['instructorudate']>$teacherquestionrec['udate']){ 
                    ...
                    <tr>
                    <td>Uploaded file</td><td align="center"> <a target="new" class="embed" href="../schoolhelp/uploads/coursematerials/<?php echo $teacherquestionrec['dlink'];?>">Click on it to Preview <span class="glyphicon glyphicon-arrow-right"></span><?php echo $teacherquestionrec['dlink'];?></a></td>
                    </tr>
                    <tr>
                    ...
                    <tr>                          
                    <td><button style="background-color:#060; border-color:#060; color:#FFF" id="cbtdelete" onclick="funcdelete('<?php echo $staffid; ?>', '<?php echo $classinfo; ?>', '<?php echo $teacherquestionrec['cm_id']; ?>', '<?php echo $teacherquestionrec['dlink']; ?>')" >Delete</button></td>

                    <td><input type="button" id="cbtedit" style="background-color:#060; border-color:#060; color:#FFF" onclick="funcedit('<?php echo $staffid; ?>','<?php echo $classinfo; ?>', '<?php echo $teacherquestionrec['cm_id']; ?>')"  value="Edit"></td>
            ...
            <div style=" width:80%; margin-left:auto; margin-right:auto; margin-bottom:50px;">
             <table style="width:100%">
                <tr>
                    <td>
                        <a class="btn btn-primary" href="?refno=<?php echo $staffid; ?>&tcid=<?php echo $classinfo; ?>&page=1">Add Course Material</a>
                    </td>
                </tr>
            </table>
         </div>
			...
			?>
           	 ...
           	 <div class="" style="background:#FFF; margin:0% 2% 2%; padding:2%; width:50%; margin-left:auto; margin-right:auto;">
                    ...
                    <tr height="23" onMouseOut="this.style.backgroundColor='';" bgcolor="#EFEFEF">
                        ...
                        <form method="post" action="?refno=<?php echo $staffid; ?>&page=2&tcid=<?php echo $classinfo; ?>" name="frmReg" onSubmit="return validatefile();" enctype="multipart/form-data">
                    <table class="table" style="border:none">
                     ...
                     <p id="error1" style="display:none; color:#FF0000;">
                        ...
                        <input class="form-control" type="file" name="coursematerials"  required="required" />
                                <!--<input class="form-control" type="file" name="coursematerials" id="file_input"  required="required" />-->
                            </td>
                        </tr>
        ...
        ?>
            ...
            <?php if ($page==3) { 
              ...
              $teachersquestions=$SHteacher->allteacheredit('coursematerials','cm_id', $recordid);
            ...
            <table class="table table-striped table-bordered table-responsive">
                    ...
                    <tr height="23"  bgcolor="#EFEFEF">
                        <td colspan="5"  align="center">
                        <form method="post" action="?refno=<?php echo $staffid; ?>&page=4&tcid=<?php echo $classinfo; ?>" name="frmedit" onSubmit="return validatefile1();" enctype="multipart/form-data">
                    <table class="table" style="border:none">
                         ...
                         <tr>
                              ...
                              <td style="padding-right:20px; border-width:thick">
                               <?php echo $csname; ?>  
                               <input id="quesid" name="cm_id" hidden="true" value="<?php echo $recordid; ?>"/>
                              </td> 
                        </tr>
                        <tr>
                         	<td> <label>Semester:</label> </td>
                      ...
                      <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="file" class="form-control" name="coursematerials" />
                                <!--<input type="file" class="form-control" name="coursematerials" id="file_input1"/>-->
                            </td>
                        </tr>
            ...
            <div style="background:#FFF; margin:0% 2% 2%; padding:2%; width:50%; margin-left:auto; margin-right:auto;">
